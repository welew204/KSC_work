...
const MONGODB_URL =
  "mongodb+srv://dbuser:UOVfndEctLfrKY1Y@ksccluster.5akj6ym.mongodb.net/";
...
app.post("/api/mongodb/:collectionName/", (request, response) => {
  ...
  db.collection(collectionName).insertOne(data, (err, results) => {
    if (err) throw err;
...
app.delete("/api/mongodb/:collectionName/", (request, response) => {
  ...
  db.collection(collectionName).deleteOne(query, (err, results) => {
    if (err) throw err;
