...
function Dashboard(props) {
  ...
  function setTaskAsCompleted(taskId) {
    ...
    const action = actions.markAsComplete(taskId);
    props.dispatch(action);
    // TODO: Challenge 5, finish this
  }

  function copyTask(taskId) {
    const action = actions.copyTask(taskId);
    props.dispatch(action);
  }
  ...
  const sum0 = tasksPhase0
    .map((task) => Number(task.points))
    .reduce((a, b) => a + b, 0);
  const sum1 = tasksPhase1
    .map((task) => Number(task.points))
    .reduce((a, b) => a + b, 0);
  const sum2 = tasksPhase2
    .map((task) => Number(task.points))
    .reduce((a, b) => a + b, 0);
  const sum3 = tasksPhase3
    .map((task) => Number(task.points))
    .reduce((a, b) => a + b, 0);
